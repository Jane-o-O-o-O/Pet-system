<template>
  <div class="page-shell admin-stats-page">
    <section class="page-head">
      <div>
        <span class="page-head__eyebrow">Analytics / Admin</span>
        <h1 class="page-head__title">经营报表与导出</h1>
        <p class="page-head__desc">查看核心经营数据，并导出寄养、疫苗、医疗用药和挂号预约的统一报表。</p>
      </div>
    </section>

    <el-row :gutter="20">
      <el-col :span="6"><el-card shadow="hover"><template #header>用户总数</template><div class="stat-value">{{ overview.users ?? '-' }}</div></el-card></el-col>
      <el-col :span="6"><el-card shadow="hover"><template #header>宠物总数</template><div class="stat-value">{{ overview.pets ?? '-' }}</div></el-card></el-col>
      <el-col :span="6"><el-card shadow="hover"><template #header>医疗记录</template><div class="stat-value">{{ overview.medicalRecords ?? '-' }}</div></el-card></el-col>
      <el-col :span="6"><el-card shadow="hover"><template #header>寄养订单</template><div class="stat-value">{{ overview.orders ?? '-' }}</div></el-card></el-col>
    </el-row>

    <el-row :gutter="20" style="margin-top: 20px">
      <el-col :span="6"><el-card shadow="hover"><template #header>待确认订单</template><div class="stat-value">{{ overview.createdOrders ?? 0 }}</div></el-card></el-col>
      <el-col :span="6"><el-card shadow="hover"><template #header>处理中订单</template><div class="stat-value">{{ overview.processingOrders ?? 0 }}</div></el-card></el-col>
      <el-col :span="6"><el-card shadow="hover"><template #header>已完成订单</template><div class="stat-value">{{ overview.completedOrders ?? 0 }}</div></el-card></el-col>
      <el-col :span="6"><el-card shadow="hover"><template #header>已取消订单</template><div class="stat-value">{{ overview.cancelledOrders ?? 0 }}</div></el-card></el-col>
    </el-row>
    ...
    <el-card style="margin-top: 20px">
      ...
      <div v-loading="boardingLoading">
        <div v-if="boardingStats">
          ...
          <el-table :data="boardingStats.orders || []" style="width: 100%; margin-top: 16px" max-height="360">
        ...
        <div v-else class="empty-tip">请选择日期范围查询</div>
      </div>
    </el-card>
...
import { formatDate, todayText } from '../../utils/date'
...
const exportStatsReport = async () => {
  ...
  link.href = url
  link.download = `pet-admin-stats-${todayText()}.csv`
